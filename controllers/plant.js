...
const getPlants = async (req, res) => {
  ...
  const allPlants = await Plant.find().sort({ updatedAt: -1, createdAt: -1 });
...
const putPlantId = async (req, res) => {
  ...
  const updatePlants = await Plant.updateOne({ _id: id }, {
    $set: {
      name: name,
      category: category,
      image: image,
  ...
  const updatedPlants = await Plant.findById(id)
